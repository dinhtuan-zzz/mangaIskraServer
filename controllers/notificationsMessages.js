...
export const getMessagesNotifications = async (req, res, next) => {
  ...
  var counts = {};
  try {
    const notification = await NotificationMessage.find({
      user: userId,
    ...
    notification.forEach(function (x) {
      counts[x.parentId._id] = (counts[x.parentId._id] || 0) + 1;
    });
...
export const readMessagesNotification = async (req, res, next) => {
  ...
  try {
    const find = await NotificationMessage.find({
      user: userId,
      parentId: targetUser,
    });
    ...
    map.forEach(
      async (item) =>
        await NotificationMessage.findByIdAndDelete(item.toString())
    );
    res.status(200).json({
      status: "success",
    });
